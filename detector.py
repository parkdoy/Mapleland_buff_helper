...
def recalibrate():
    """Handles local minimap calibration and saves the result."""
    global MINIMAP_COORDS
    print("로컬 미니맵 재설정 요청을 받았습니다...")
    ...
    if new_coords:
        ...
        try:
            ...
            print(f"미니맵 설정 완료 및 저장: {new_coords}")
        except IOError as e:
            print(f"오류: minimap_config.py 파일 저장에 실패했습니다: {e}")
        
        # Start background threads if they haven't been started yet
        start_background_threads()
        
        # Return the coordinates so the UI can use them
